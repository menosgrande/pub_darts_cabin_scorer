...
**バグ修正: 通算PPD/MPRが各試合の値の単純平均になっていた（修正済み）**

`summarizePlayerStats`は当初、各試合の`ppd`/`mpr`を`ppdSum += r.ppd`のように単純合計してから件数で割っていた。これは「試合ごとのダーツ本数・ラウンド数の違いを無視した平均」になっており、例えば3ダーツだけの短い試合(PPD60)と300ダーツの長い試合(PPD10)が半々の重みで平均され、実態(総得点÷総ダーツ数で計算すると本来PPD10.5程度)とかけ離れた通算値になっていた。

`buildGameStatsRecords`に生の`scored`(01: 消費点数、Count-Up: 累積得点)と`marks`(cricket: 通算マーク数)を追加し、`summarizePlayerStats`側は`総得点÷総ダーツ数`・`総マーク数÷総ラウンド数`で計算するよう修正。`scored`/`marks`が無い(移行前の)古いレコードに対しては`ppd*darts`/`mpr*rounds`で復元するフォールバックを入れ、後方互換性を保っている。「ベストPPD/ベストMPR」(試合単位の最大値)はこの問題の対象外なので変更していない。`tests/gameStats.test.js`にダーツ数が偏った試合を混ぜた回帰テストを追加(単純平均だと35になるところを正しく10.49...になることを確認)。

**教訓**: 「平均のようなものを複数の試合にまたがって集計する」場合、単純に各試合の平均値同士を平均すると、試合ごとの母数(このケースではダーツ本数・ラウンド数)の違いが無視され、統計として意味のない値になる。合計すべきは「平均された値」ではなく、その前段階の「合計値(得点・マーク数)と合計母数(本数・ラウンド数)」であるべきで、割り算は最後に1回だけ行う。
...
**モード切り替えの土台（実装済み）**:
`inputMode`（`"board"|"camera"`、デフォルト`"board"`）というstateを追加し、ゲーム画面ヘッダーのカメラアイコンボタンでトグルできる。盤面SVG（w-42%の円形コンテナ）を`inputMode`に応じて、既存の盤面タップUIか`CameraInputPanel`（`js/camera/camera-input.js`。ゲームロジック本体からは独立した入力アダプター用フォルダとして新設）のどちらかに丸ごと差し替える構造。`CameraInputPanel`は現時点では`getUserMedia()`で映像を表示するだけのプレースホルダーで、ダーツ検出・座標変換・キャリブレーションはまだ実装していない。「盤面タップに戻す」ボタンでいつでも`inputMode="board"`に戻せる。読み込み順は`ui-components.js`の直後・`app-main.js`の直前（`index.html`参照）。

**1台/2台構成の切り替え（実装済み）**:
`CameraInputPanel`は`cameraCount`(1|2)を持ち、`navigator.mediaDevices.enumerateDevices()`で列挙したカメラの中からスロットごとにデバイスを選べる（`CameraSlot`コンポーネントが1台分の映像表示・ストリーム開閉を担当）。`enumerateDevices()`はラベル取得に事前の許可が必要なため、マウント時に一度`getUserMedia({video:true})`で権限を取ってからすぐ閉じ、その後に列挙している。同じデバイスが両スロットに選ばれている場合は警告表示のみ行う（禁止はしない）。

現時点では「2台目の映像をどう検出に使うか」（視差による誤検出の抑制など）は未設計。今後dart-detection.jsを実装する段階で、1台構成時と2台構成時でロジックを分岐させる必要がある。1台構成のロジックを先に完成させ、2台構成はその上に「2つの検出結果の一致度を見て信頼度を上げる」形で拡張するのが素直（2台目必須の設計にはしない）。

`boardRef`を参照する`handleBoardClick`は`if (!boardRef.current) return;`で既にガードされているため、カメラモードで盤面SVGが未マウントでも安全（新規に追加したガードではなく、既存の防御がそのまま機能した）。

次にやること: `CameraInputPanel`に①手動4点キャリブレーションUI（映像上をタップしてホモグラフィ行列を計算）→②`getThrowFromCoords()`との接続テスト→③ダーツ検出（差分検出、Dart Sense方式の「ボード基準点検出→射影変換→ダーツ位置検出」を構造の参考にする。コード/学習済みモデルはライセンス上コピーしない）、の順で積み上げる。

**①②実装済み（`js/camera/calibration.js`）**:
- `computeHomography(srcPoints, dstPoints)` / `applyHomography(H, x, y)`: 4点対応からのホモグラフィ計算(ガウスの消去法による8元連立方程式の直接解、外部ライブラリ不要)。純粋関数として`tests/calibration.test.js`でカバー(往復変換・平行移動+スケーリングの復元・縮退時のエラー・`getThrowFromCoords`との接続まで一貫したテストを含む、計10件)
- 基準点は`CALIBRATION_BOARD_POINTS`（盤面座標系でダブルリング外周=`BOARD_OUTER_RADIUS`の12時/3時/6時/9時）に固定。「何番のセグメントか数える」負担をユーザーに強いない設計
- `BOARD_OUTER_RADIUS`をconstants.jsに新設し、checkout.jsの`rDoubleOuter`もこれを参照するよう統一（キャリブレーションと盤面判定の基準半径が別々の場所で定義されて食い違う、以前実際に起きたのと同種のバグを構造的に防ぐ）
- `CalibrationOverlay`コンポーネント（映像上に4点タップのガイドを表示、`CameraSlot`に組み込み済み）。キャリブレーション結果は`CAMERA_STORAGE_KEY`にdeviceId単位で保存し、キャビン設置(カメラが動かない前提)なら次回以降やり直し不要
- キャリブレーション済みなら映像タップで`getThrowFromCoords`の判定結果(S/D/T/ラベル/点数)をその場で確認できる「テストモード」も同じ`CameraSlot`に実装済み(④の実機検証の前段階)

**未着手**: ③ダーツ検出（差分検出）、実機でのキャリブレーション精度検証、2台構成時の情報の使い方（前セクション参照）
...
### fat(50/50)設定の外側ブルが、Double Outでは実際にBUSTになっていた（修正済み）

`getThrowFromCoords`はfat設定の外側ブルを`{score:50, multiplier:1, label:"Bull(50)", isBull:true}`として返す(score×multiplier=50を保つため、常にmultiplier:1で返す設計)。一方`getRoundState`のDouble Out判定は`t.multiplier===2 || isDBull(label==="D-Bull")`だけを見ていたため、外側ブルは`multiplier:1`・`label:"Bull(50)"`のどちらの条件にも合致せず、**残り50でfat外側ブルを踏んで0にしても、Double OutではBUSTになっていた**。しかも`getFinishTargets`（盤面が光る機能）はこのケースを「有効なフィニッシュ」として無条件にハイライトしていたため、**「ここを狙えば上がれます」と表示された場所に実際に刺すとBUSTになる**という、UIと採点エンジンが矛盾する状態だった。`findCheckoutRoute`（チェックアウトルート提案）側も同じ`multiplier===2`基準だったため、そちらはfat外側ブルを候補に出さない(=単に提案しない)という、より軽微な形で同じズレを抱えていた。

**修正内容**: `getRoundState`のDouble Out判定に`isFatOuterBull(label==="Bull(50)")`を追加して有効化。`computeCheckoutRoute`の`isValidOut`/`finish1`も同じ基準に揃え、ついでにラベル表記の誤り（候補生成側で"Bull"/"S-Bull"としていたのを、実際の`getThrowFromCoords`の返り値と一致する"Bull(50)"/"S-Bull(25)"に修正）も直した。あわせて`getFinishTargets`側で見つかった関連の漏れ（separate設定・残り25・Master Outで、外側ブルが有効なフィニッシュのはずなのに光っていなかった。旧コードは`allowSingle`のみを条件にしており、Master Outを見落としていた）も同時に修正。`tests/fatBullDoubleOut.test.js`に、実際の`getThrowFromCoords`の返り値を使った（モックを書かない）回帰テストを15件追加。

**教訓**: 「score×multiplier=点数」という等式を保つために、意味的にはダブル相当(50点/2倍の代替)なのにmultiplierフィールドは1で返す、という設計をした箇所（D-Bull・fat外側ブル）は、`multiplier===2`という素直な条件だけでは判定漏れが起きる。得点系のフィールドを「見た目の意味」と「計算上の値」で使い分けている箇所は、判定ロジック側で明示的にラベル/フラグを見て特別扱いする必要があり、しかもその特別扱いは判定を行う関数(`getRoundState`)・アシスト関数(`findCheckoutRoute`)・ハイライト関数(`getFinishTargets`)の**3箇所すべて**で一致させないと、今回のような「UIは上がれると言うが実際はBUST」が発生する。今回のように1つの外部情報源から3箇所同時に指摘を受けたのは珍しいが、次に似た構造(スコアとmultiplierが素直に対応しない特殊ケース)を追加するときは、関連する3関数を横断してチェックリスト的に確認すること。

### 追記: ルール判定に表示ラベル文字列を使っていた（修正済み）

上記の修正で導入した`isDBull = t.label === "D-Bull"`・`isFatOuterBull = t.label === "Bull(50)"`は、動作としては正しかったが、**表示用の文字列(`label`)をゲームルール判定の識別子として使ってしまっていた**。`getHitSoundType`の`t.label.includes("Bull")`も同じパターン。この設計だと、将来UIの表示文言（例えば日本語化やラベル文言の調整）を変えただけで、採点ロジックまで無関係に壊れるリスクがある。

**修正内容**: `getThrowFromCoords`のブル判定に、表示に依存しない構造化フィールド`bullRing`("inner"|"outer")を追加。`getRoundState`のDouble Out判定は`t.label`を一切見ず、`t.isBull && t.score === 50`（=構造化データだけ）で判定するように変更（D-Bullは常に50点、fat設定の外側ブルも50点、separate設定の外側ブルは25点という既存の値の違いだけで十分区別できるため、`bullRing`フィールド自体は今回のロジックには不要だったが、将来カメラ検出やUIが「どちらの物理リングに当たったか」を必要とする場合に備えて構造化データとして残してある）。`getHitSoundType`の`label.includes("Bull")`も`t.isBull`のみの判定に変更。
