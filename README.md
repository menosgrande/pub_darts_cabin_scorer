...
- **CPU対戦**: EASY〜PRO の4段階の難易度でAIと対戦可能。各難易度の傾向を一言で表示
...
2. セットアップ画面は上から「PLAYERS」→「QUICK START」→「GAME MODE」→「RULES」→「OK」の順。とりあえず遊びたいだけならプレイヤー数を選んでクイックスタートの「501」「CRICKET」「COUNT-UP」のいずれかを押すだけでOK
...
盤面上のピンチズームは無効化しています（フィルター効果を多用したSVGのため、モバイルブラウザでのネイティブズーム連打がクラッシュの原因になることが確認されたため）。ページ全体のズームは引き続き可能です。
...
- **MANUAL**:
  ...
  - クリケット: 「編集中: P1 ⇄ P2」で表示するプレイヤーを切り替えながら、20・19・18・17・16・15・BULLそれぞれに▲▼ボタンで0〜3マークを個別指定。P1/P2は独立して設定可能（両方に付けることもできます）。得点の直接加算（+8刻み）も可能
